...
parser = argparse.ArgumentParser(
    description='EdgeReMIND LinkPropPred Example for knowledge graphs',
...
def eval(
    ...
    for batch in tqdm(loader):
        ...
        for idx, neg_batch in enumerate(batch.neg_batch_list):
            n = len(neg_batch) + 1
            query_src = batch.edge_src[idx].repeat(n)
            query_dst = torch.cat([batch.edge_dst[idx].unsqueeze(0), neg_batch])
            query_ts = batch.edge_time[idx].repeat(n)
            query_type = batch.edge_type[idx].repeat(n)

            y_pred = model(query_src, query_dst, query_ts, query_type)
            input_dict = {
...
if __name__ == '__main__':
    ...
    val_hook = TGBTKGNegativeEdgeSamplerHook(
        args.dataset,
        split_mode='val',
        first_dst_id=min_dst_node,
        last_dst_id=max_dst_node,
    )
    test_hook = TGBTKGNegativeEdgeSamplerHook(
        args.dataset,
        split_mode='test',
        first_dst_id=min_dst_node,
        last_dst_id=max_dst_node,
    )
    if args.dataset in ('tkgl-wikidata',):
        val_hook.neg_sampler.strategy = 'dst-time-filtered'
        test_hook.neg_sampler.strategy = 'dst-time-filtered'
    hm.register('val', val_hook)
    hm.register('test', test_hook)
    ...
    trainer = EdgeReMINDTrainer(
        model,
        first_dst=int(min_dst_node),
        last_dst=int(max_dst_node),
        k=20,
        feature_bsize=200,
